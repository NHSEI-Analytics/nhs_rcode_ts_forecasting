...
file_data <- "sample_test_data.csv"
...
fun_fc_model_is <- function(x_vals, x_dates, modl, horiz, freq) { 
  ...
  df_model = data.frame(metric_period = x_dates, metric_value = x_vals, metric_type = "data", 
                        lo80 = NA, up80 = NA, lo95 = NA, up95 = NA, model = NA, stringsAsFactors = FALSE)
  df_fc = data.frame(metric_period = fc_dates, metric_value = unclass(fc_model$mean), metric_type = "fc",  
...
fun_fc_model_oos <- function(x_vals, x_dates, modl, hold_out, freq) { 
  ...
  df_train = data.frame(metric_period = x_train_dates, actual_value = x_train, fc_value = NA, 
                        lo80 = NA,up80 = NA,lo95 = NA,up95 = NA, model = NA, stringsAsFactors = FALSE)
  df_test = data.frame(metric_period = x_test_dates, actual_value = x_test, fc_value = unclass(fc_model$mean),  
                       lo80 = unclass(fc_model$lower)[,1], up80 = unclass(fc_model$upper)[,1],
                       lo95 = unclass(fc_model$lower)[,2], up95 = unclass(fc_model$upper)[,2], 
                       model = NA, stringsAsFactors = FALSE) 
...
df_x <- df_import_data %>% mutate(metric_value = round(metric_value + 42 + rnorm(nrow(df_import_data), 5, 2))) 

write.table(df_x, "clipboard-64000", sep = "\t", row.names = FALSE)
...
### assign metric_month = metric_period to maintain backwards compatibility with v3 functions
df_fc_oos <- df_fc_oos %>% mutate(metric_month = metric_period, model_type = str_extract(model, "^[^\\(]+"), 
                                  model_type = fun_trim(model_type), 
                                  model_type = if_else(model_type == " BATS", "TBATS", model_type)) %>% select(-metric_period)
...
df_fc_final <-  df_fc_final %>% mutate(metric_month = metric_period, model_type = str_extract(model,"^[^\\(]+"), 
                                       model_type = fun_trim(model_type), 
                                       model_type = if_else(model_type=="BATS", "TBATS", model_type)) %>% select(-metric_period)
